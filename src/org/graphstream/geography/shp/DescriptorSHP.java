...
public class DescriptorSHP extends Descriptor {
	...
	public boolean isPoint(Object o) {
		...
		return (feature.getType().getGeometryDescriptor().getType()
				.getBinding() == com.vividsolutions.jts.geom.Point.class);

	}

	protected boolean isPolyline(SimpleFeature feature) {
		// TODO: A better way to do this?
		Class<?> binding = feature.getType().getGeometryDescriptor().getType()
				.getBinding();
	...
	public Point newPoint(Object o) {
		...
		Coordinate[] c = ((Geometry) feature.getDefaultGeometry())
				.getCoordinates();
		...
		for(Property property : properties)
			if(this.filter == null
					|| this.filter.isKept(property.getName().toString()))
				point.addAttribute(property.getName().toString(),
						property.getValue());
